...
webrtc::PacingController::Configuration LowLatencyPacingConfiguration(
  ...
  configuration.keyframe_flushing = false;
  // A bounded queue is never drained faster than the configured pacing rate;
  // producers observe ExpectedQueueTime() and back off instead.
  configuration.drain_large_queues = !bounded_queue;
...
PacedSender::PacedSender(std::shared_ptr<webrtc::Clock> clock,
...
void PacedSender::MaybeProcessPackets(
  ...
  if (!time_to_next_process.IsFinite() ||
      time_to_next_process > webrtc::PacingController::kPausedProcessInterval) {
    // Nothing scheduled (or an infinite send time): poll at the keep-alive
    // interval instead of posting a delay the task queue cannot represent.
    time_to_next_process = webrtc::PacingController::kPausedProcessInterval;
  }
...
int PacedSender::EnqueueRtpPackets(
  ...
  for (auto &rtp_packet : rtp_packets) {
    ...
    // Reliable (KCP) control traffic is small and latency sensitive: give it
    // the top priority level. Bulk unreliable data is paced at normal level.
    if (rtp_packet_to_send->PayloadType() == rtp::PAYLOAD_TYPE::KCP) {
