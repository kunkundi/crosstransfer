...
class TaskQueue {
 public:
  ...
  bool PostDelayedTask(AnyInvocable<void()> task, int delay_ms) {
    ...
    if (delay_ms > 3600 * 1000) delay_ms = 3600 * 1000;
  ...
  bool PostDelayedHighPrecisionTask(AnyInvocable<void()> task,
    ...
    // Guard against non-finite / absurd delays overflowing the time point.
    constexpr auto kMaxDelay = std::chrono::hours(1);
    if (delay < std::chrono::microseconds(0)) delay = std::chrono::microseconds(0);
