...
class SrtpEngine {
 public:
  struct Params {
    ...
    // Sender side: one session protecting every outbound SSRC (RTP + RTCP).
    bool sender_any_outbound = false;
  };
  ...
  class SrtpSession {
   public:
    ...
    // Same contract for RTCP (SRTCP adds a 4-byte index + 16-byte tag).
    int protectRtcp(uint8_t* buf, int* len) const;
    int unprotectRtcp(uint8_t* buf, int* len) const;
  ...
  // Create a sender SrtpSession bound to a specific SSRC, or to any outbound
  // SSRC when p.sender_any_outbound is set.
