...
SrtpEngine::SrtpSession SrtpEngine::CreateSender(const Params& p) {
  Params cp = p;
  cp.receiver_any_inbound = false;
  if (cp.ssrc == 0 && !cp.sender_any_outbound) {
...
int SrtpEngine::SrtpSession::protectRtcp(uint8_t* buf, int* len) const {
  if (!session_) return -static_cast<int>(srtp_err_status_no_ctx);
  if (!buf || !len || *len <= 0) return -static_cast<int>(srtp_err_status_bad_param);
  return -static_cast<int>(srtp_protect_rtcp(session_, buf, len));
}

int SrtpEngine::SrtpSession::unprotectRtcp(uint8_t* buf, int* len) const {
  if (!session_) return -static_cast<int>(srtp_err_status_no_ctx);
  if (!buf || !len || *len <= 0) return -static_cast<int>(srtp_err_status_bad_param);
  return -static_cast<int>(srtp_unprotect_rtcp(session_, buf, len));
}
