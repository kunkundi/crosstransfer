...
桌面交互优化（2026-09-23）：取件码结果页直接显示当前分享各接收端的进度、速度与剩余时间，单独取消某次发送不关闭整个分享；没有进行中传输时保留最近一次结果反馈，无须进入记录查看。结果页同时显示取件码剩余有效期，复制取件码为主操作，复制链接与二维码为次级入口；复制反馈在按钮原位显示 2 秒。二维码使用独立弹窗，分享结束或过期即隐藏二维码，过期不打断已建立的传输。
