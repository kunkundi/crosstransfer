...
- 通知中心、未读标记与离线补收；服务端管理后台支持发布、撤回和持久化运营通知。
- 桌面端提供托盘快速投递篮，可拖入文件或目录并直接生成取件码。
- 手机系统分享、原生扫码、文件导出、导入副本清理；Android 前台传输服务，iOS 有限后台执行时间。
- 发行方统一运营 Go 服务，客户端自动连接；无账号、不存文件。第一版信任服务器转发的指纹，尚未实现 SPAKE2 口令认证。
...
- [内部服务部署、TLS、ACME 与手机链接](docs/SELF_HOSTING.md)
...
商用客户端的服务由发行方在构建时指定，不向终端用户开放配置；域名待定。见[客户端服务发行配置](docs/CLIENT_SERVICE.md)。上述回环命令仅供内部开发使用。
