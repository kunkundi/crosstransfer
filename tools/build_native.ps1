...
try {
    $Developer = if ($env:CT_DEVELOPER_MODE) { $env:CT_DEVELOPER_MODE } else { 'n' }
    Invoke-Xmake -Arguments @('f', "--ct_developer=$Developer", "--ct_service_host=$($env:CT_SERVICE_HOST)", "--ct_link_host=$($env:CT_LINK_HOST)", '-p', 'windows', '-a', $Arch, '-m', $Mode, '--ct_native=y', '-o', 'build', '-y')
